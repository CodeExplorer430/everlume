...
export function MemorialConsentLog({ memorialId }: MemorialConsentLogProps) {
  ...
  const revokeConsent = async () => {
    ...
    const response = await fetch(`/api/admin/memorials/${memorialId}/media-consent`, {
      method: 'POST',
    })

    if (!response.ok) {
      const payload = (await response.json().catch(() => null)) as { message?: string } | null
      setErrorMessage(payload?.message || 'Unable to revoke protected media consent.')
      setRevoking(false)
      return
    }
    ...
    setSuccessMessage('Existing protected-media consent cookies were revoked for this memorial.')
    setRevoking(false)
  }
    ...
    <section className="surface-card space-y-4 p-6">
      <div className="flex flex-col gap-3 border-b border-border pb-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1">
          ...
          <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">Active notice version {noticeVersion}</p>
          {revokedAt ? <p className="text-xs text-muted-foreground">Last revoked: {new Date(revokedAt).toLocaleString()}</p> : null}
        </div>
        <button
          type="button"
              ...
              {entries.map((entry) => (
                <tr key={entry.id} className="align-top">
                  ...
                  <td className="py-3 pr-4 capitalize text-muted-foreground">
                    {entry.access_mode}
                    {entry.consent_version ? <span className="block text-xs uppercase tracking-[0.14em] text-muted-foreground">v{entry.consent_version}</span> : null}
                  </td>
