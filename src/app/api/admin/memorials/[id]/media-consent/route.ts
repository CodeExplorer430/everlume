...
export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: memorial }, { data: siteSettings }] = await Promise.all([
    supabase.from('pages').select('id, title, media_consent_revoked_at').eq('id', parsed.data.id).single(),
    supabase.from('site_settings').select('protected_media_consent_version').eq('id', 1).single(),
  ])

  const { data, error } = await supabase
    .from('media_access_consents')
    .select('id, event_type, access_mode, consent_source, consent_version, media_kind, media_variant, ip_hash, user_agent_hash, created_at')
  ...
  return NextResponse.json(
    {
      ...
      consentNoticeVersion: Number(siteSettings?.protected_media_consent_version) || 1,
    },
    { status: 200 }
  )
}
